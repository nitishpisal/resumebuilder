...
def nextskil():
    global i, skil, stitle, sdetail, adde, gen, nextedu
    adde.destroy()
    gen.destroy()
    nextedu.destroy()
    stitle = [''] * 5
...
def gener():
    global name, addres, headln, wl2, emailid, phonee, weblink, univ, school, gpa, loc, course, major, datee, exp, edu, adde, gen, nextedu, company, locexp, posi, startdate, enddate, detailss, skil, stitle, sdetail
    gname = name.get()
    gaddres = addres.get()
    gwl2 = wl2.get()
    gemail = emailid.get()
